...
def render(scored: list[dict[str, Any]], purge_plan: dict[str, Any], run_dir: Path) -> None:
    ...
    manifest = []
    for game in active:
        summary = sanitize_public_text(str(game.get("summary") or game.get("prompt") or ""))
        manifest.append({
            "id": game.get("id"),
            "title": sanitize_public_text(str(game.get("title") or game.get("id"))),
            "prompt": summary,
            "summary": summary,
    ...
    for idx, game in enumerate(manifest, start=1):
        ...
        summary = html.escape(str(game.get("summary") or "")[:260])
        url = html.escape(str(game.get("url")))
        cards.append(f"<article class='game-card'><div class='rank'>#{idx:02d}</div><div><h2>{title}</h2><p>{summary}</p><p class='score'>Score {game.get('score', 0)}</p><a class='play' href='{url}'>Play build</a></div></article>")
    latest_url = html.escape(str(manifest[0]["url"])) if manifest else "#"
    index = "<!doctype html><html lang='en'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>NexusRealtime KitBuilder03</title><link rel='stylesheet' href='./launcher.css'></head><body><main class='shell'><section class='hero'><p class='eyebrow'>rolling frontier</p><h1>KitBuilder03 Active Builds</h1><p class='lede'>The public gallery keeps the best ten builds and carries capabilities forward.</p><a class='primary' href='" + latest_url + "'>Play top build</a><a class='secondary' href='./cleanup.html'>Review cleanup ledger</a></section><section class='ladder'>" + (''.join(cards) if cards else '<p>No active builds yet.</p>') + "</section></main></body></html>"
