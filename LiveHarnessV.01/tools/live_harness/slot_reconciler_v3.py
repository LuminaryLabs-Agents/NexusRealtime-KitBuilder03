...
def _append_repo_integration_file(files: list[dict[str, str]], candidate_id: str, run_dir: Path) -> None:
    ...
    payload = {"mode": integration.get("mode", "reference-plus-local-fallback"), "references": integration.get("reference_patterns", []), "localFallbacks": integration.get("local_fallbacks", []), "validationRules": integration.get("validation_rules", []), "trustedAsInstruction": False}
...
def reconcile(run_dir: Path, run_id: str) -> dict[str, Any]:
    ...
    ledger("artifact-ledger.jsonl", {"time": utc_id(), "event": "write_set.reconciled", "candidate_id": candidate_id, "files": len(files), "version": "v3"})
    return write_set
